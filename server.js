...
app.get('/api/health', (req, res) => {
    res.json({ status: 'ok', message: 'Halal AI Trading Bot - Real + Demo' });
});
...
async function placeRealMarketOrder(apiKey, secretKey, symbol, side, quoteOrderQty, useTestnet = false) {
...
// ==================== API KEY MANAGEMENT (REAL + TESTNET) ====================
...
app.get('/api/admin/user-balances', authenticate, async (req, res) => {
    ...
    for (const [email, userData] of Object.entries(users)) {
        ...
        try {
            ...
            // For real balances, we need to know if user is using testnet or real
            // This is simplified - in production, store accountType per user
...
class AutoCompoundingEngine {
    async executeTrade(sessionId, userEmail, apiKey, secretKey, config, useTestnet = false) {
        ...
        const minTrade = 10;
        if (positionSize < minTrade) positionSize = minTrade;
        ...
        try {
            const order = await placeRealMarketOrder(apiKey, secretKey, symbol, isBuy ? 'BUY' : 'SELL', positionSize, useTestnet);
            ...
            console.log(`📊 Trade: ${isBuy ? 'BUY' : 'SELL'} $${positionSize.toFixed(2)} ${symbol} - Profit: $${profit.toFixed(2)}`);
...
app.post('/api/start-trading', authenticate, async (req, res) => {
    ...
    try {
        ...
        if (!useTestnet && balance < 10) {
            return res.status(400).json({ success: false, message: `Real trading requires minimum 10 USDT. You have ${balance} USDT.` });
        }
    } catch (error) {
    ...
    res.json({ 
        success: true, 
        sessionId, 
        message: `🚀 ${mode.toUpperCase()} TRADING STARTED! Target: ${(targetProfit/initialInvestment).toFixed(0)}x in ${timeLimit} hour(s) (Trade every ${tradeIntervalSeconds}s)`
    });
});
...
app.post('/api/get-balance', authenticate, async (req, res) => {
    ...
    try {
        ...
        const useTestnet = (accountType === 'testnet');
        const balance = await getRealBalance(apiKey, secretKey, useTestnet);
        res.json({ success: true, spotBalance: balance, fundingBalance: 0, total: balance });
    } catch (error) {
...
app.listen(PORT, '0.0.0.0', () => {
    console.log(`\n🌙 HALAL AI TRADING BOT - REAL + DEMO READY`);
    console.log(`✅ Owner: mujtabahatif@gmail.com / Mujtabah@2598`);
    console.log(`✅ Supports: Real Binance + Testnet/Demo`);
    console.log(`✅ Minimum balance required: 10 USDT for Real mode`);
